...
def question_cat(a, cat1_ind, cat2_ind, thres=8):
    '''
    categorize the question for a batch of answer 
    cat1_ind: index for Y/N
    cat2_ind: index for Numerical 
    '''
    cat1 = a[:,cat1_ind].sum(dim=1) >= thres
    cat2 = a[:,cat2_ind].sum(dim=1) >= thres
    cat3 = torch.logical_not(torch.logical_or(cat1, cat2))
    return cat1, cat2, cat3


def batch_accuracy_2(predicted, true):
    """ Compute the accuracies for a batch of predictions and answers
    acc = min {# humans that said ans / 3, 1}
    """
    _, predicted_index = predicted.max(dim=1, keepdim=True)
    agreeing = true.gather(dim=1, index=predicted_index)
    return (agreeing * 0.3333).clamp(max=1)
...
def get_transform(target_size, central_fraction=1.0, pad = 168):
    return transforms.Compose([
        transforms.Pad((0, pad)),

...
class VQA(data.Dataset):
    ...
    def __init__(self, questions_path, answers_path, image_features_path, answerable_only=False):
        ...
        #$ filter the images with id record in the feature file
        with h5py.File(self.image_features_path, 'r') as features_file: 
            coco_ids = features_file['ids'][()]
        filter_index = [i for i in range(len(questions_json['questions'])) if questions_json['questions'][i]['image_id'] in coco_ids]
        self.answers = [self.answers[i] for i in filter_index]
        self.questions = [self.questions[i] for i in filter_index]
        self.coco_ids = [self.coco_ids[i] for i in filter_index]
...
class CocoImages(data.Dataset):
    ...
    def __init__(self, path, transform=None):
        ...
        self.path = path
        if self.path.split('/')[-1][:5] == 'train':
            self.max_image = config.max_image_train
        else:
            self.max_image = config.max_image_val
    ...
    def _find_images(self):
        ...
        for filename in os.listdir(self.path):
            if not filename.endswith('.png') or k >= self.max_image:  #$ only select the first 1000 images
                break
            k += 1
            ...
            try:
                ...
                print("not valid id {}".format(id_and_extension))
                pass
